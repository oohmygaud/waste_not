...
- Create a copy of ``waste_not/settings/local.py.example``:  
  `cp waste_not/settings/local.py.example waste_not/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/waste_not/settings/base.py``.
- Adjust ``backend/waste_not/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
